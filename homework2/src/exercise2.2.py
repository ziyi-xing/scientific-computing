...
#         # 运行蒙特卡洛模拟并将结果保存在 dla_results 中
#         dla.MonteCarlo()
#         dla_results.append(dla)

#     # 绘制最终帧图像和保存动画
#     for i, dla in enumerate(dla_results):
#         # 获取最后一帧
#         final_frame = dla.history[-1][::-1, :]  # 获取最后一帧
...
#         ax.imshow(final_frame, cmap='viridis', vmin=0, vmax=1, origin='lower')
#         step_when_top_reached = len(dla.history)
#         ax.set_title(f"ps={ps_values[i]}, Step={step_when_top_reached}")  # 设置标题

#         # 生成动画并获取 ani 对象
#         ani = dla.animate()

#         # 保存动画为 GIF
#         gif_path = os.path.join(figures_dir, f"dla_MCanimation_ps_{ps_values[i]}.gif")  # 动态生成文件名
#         ani.save(gif_path, writer="pillow", fps=15)  # 保存为 GIF，fps 控制帧率
#         print(f"Animation saved to {gif_path}")  # 打印 GIF 保存的路径
